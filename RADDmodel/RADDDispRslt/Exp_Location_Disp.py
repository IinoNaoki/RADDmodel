...
RESset_sidernd = pickle.load(open("../results/LocDistr_changing/sidernd","r"))
...
xlabel('Transition matrix of state $\mathcal{L}$',fontsize=16)
...
ylim([-24,10])
plt.xticks((1,2,3),('Matrix\n$\mathbf{W}_{L1}$', 'Matrix\n$\mathbf{W}_{L2}$', 'Matrix\n$\mathbf{W}_{L3}$',) )
xlim([0.90, 3.10])
...
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
# legend(loc='best', ncol=1,fancybox=True,shadow=True)
legend(loc=(0.55, 0.17), fancybox=True)
ylim([-0.005,0.16])
plt.xticks((1,2,3),('Matrix\n$\mathbf{W}_{L1}$', 'Matrix\n$\mathbf{W}_{L2}$', 'Matrix\n$\mathbf{W}_{L3}$',) )
xlim([0.90, 3.10])
locs, labels = plt.yticks()
