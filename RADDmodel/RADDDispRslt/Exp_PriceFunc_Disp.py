...
RESset_sidernd = pickle.load(open("../results/PriceFunc_changing/sidernd","r"))
...
xlabel('Price distribution of electricity charger',fontsize=14)
...
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
# legend(loc='best', ncol=1,fancybox=True,shadow=True)
legend(loc=(0.1,0.47), fancybox=True)
# ylim([-0.05,1.30])
plt.xticks((1,2,3),('Low\nprice', 'Medium\nprice', 'High\nprice') )
xlim([0.90, 3.10])
locs, labels = plt.yticks()
...
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
# legend(loc='best', ncol=1,fancybox=True,shadow=True)
legend(loc=(0.1, 0.25), fancybox=True)
ylim([-0.01,0.26])
plt.xticks((1,2,3),('Low\nprice', 'Medium\nprice', 'High\nprice') )
xlim([0.90, 3.10])
locs, labels = plt.yticks()
